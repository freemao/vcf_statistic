...
msg_usage = 'usage: %prog [-M] geno, var [-F] vcffile'
descr ='''count the genotype and variant type in each seq
 '''
optparser = OptionParser(usage = msg_usage, description = descr)
optparser.add_option('-M', '--model', dest = 'model',type = 'string',
                       help = 'count genotype or variant type.')
optparser.add_option('-F', '--file', dest = 'vcffile',type = 'string',
                       help = 'count genotype or variant type.')
options, args = optparser.parse_args()

def GenoType_statistic(vcfname):
...
def VariantType_fb_statistic(vcfname):
...
if __name__ == '__main__':
    ...
    f = options.vcffile
    if m == 'geno':
        GenoType_statistic(f)
    if m == 'var':
        VariantType_fb_statistic(f)
